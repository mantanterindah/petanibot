...
module.exports = async (client, interaction) => {
    if (!interaction.isCommand() && !interaction.isContextMenu() && !interaction.isButton()) return; // When the interaction is not a command, not a contextmenu, or not a button, it will not execute.

    if (interaction.isCommand() || interaction.isContextMenu()) { // If the command is a command or an contextmenu, it will run the below code.
